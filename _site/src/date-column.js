define(function(require, exports, module) {
  ...
  <table class="ali-calendar-date" data-role="date-column">
    <tr class="ali-calendar-day-column">
      {{#each day.items}}
      <th class="ali-calendar-day ali-calendar-day-{{value}}" data-role="day" data-value="{{value}}">{{_ label}}</th>
      {{/each}}
    </tr>
    {{#each date.items}}
    <tr class="ali-calendar-date-column">
      {{#each this}}
      <td class="ali-calendar-day-{{day}} {{className}} {{#unless available}}disabled-element{{/unless}}" data-role="date" data-value="{{value}}">{{label}}</td>
      {{/each}}
    </tr>
    {{/each}}
  </table>
  */
  ...
  function template(model, options) {
    ...
    var html = '<table class="ali-calendar-date" data-role="date-column">';

    // day column
    html += '<tr class="ali-calendar-day-column">';
    $.each(model.day.items, function(i, item) {
      html += '<th class="ali-calendar-day ali-calendar-day-' + item.value + '" ';
    ...
    $.each(model.date.items, function(i, items) {
      html += '<tr class="ali-calendar-date-column">'
      $.each(items, function(i, item) {
        var className = [
          'ali-calendar-day-' + item.day,
          item.className || ''
        ];
        if (!item.available) {
